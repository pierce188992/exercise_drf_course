...
(venv) C:\Users\pierce\Desktop\Python\web_development\django_rest_framework\drf_course\backend>deactivate

**
"Package 'netcat' has no installation candidate" and that was halting the whole process. Apparently you need to change "apt-get -y install netcat" to "apt-get -y install netcat-traditional" in both of the files Dockerfile and Dockerfile_app (found in the /backend/docker/docker_files folder),
**
In the Dockerfile remove the contents in ENTRYPOINT. ENTRYPOINT[ ]

## C:\Users\pierce\Desktop\Python\web_development\django_rest_framework\drf_course>docker-compose up -d --build

---

8. Call our endpoints - Here are the requests we can make to our new endpoint.

to "drf_course-app-1" terminal :
